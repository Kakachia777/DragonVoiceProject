...
class DragonVoiceGUI:
    ...
    def create_tabs(self):
        ...
        self.tab_container.pack(
            ...
            expand=True,
            padx=20,
            pady=(0, 10))  # Reduced bottom padding from 20 to 10
        ...
        self.tab_view = ctk.CTkTabview(
            ...
            segmented_button_unselected_color=self.colors["bg_medium"],
            segmented_button_unselected_hover_color=self.colors["bg_light"])
        self.tab_view.pack(fill="both", expand=True, pady=0)  # Added pady=0 to remove any default padding
        
        # Reduce the internal padding of the tab view
        for tab_name in ["Dashboard", "Chatbots", "History", "Configuration", "About"]:
            self.tab_view.add(tab_name)
            # Apply minimal padding to the tab content
            self.tab_view.tab(tab_name).configure(fg_color=self.colors["bg_dark"])
    ...
    def create_dashboard_tab(self):
        ...
        # Main container with gradient background - make it scrollable
        dashboard_container = ctk.CTkScrollableFrame(
            self.tab_view.tab("Dashboard"),
            fg_color="transparent"
        )
        dashboard_container.pack(fill="both", expand=False, padx=10, pady=5)  # Reduced padding from 20 to 5
        ...
        status_panel.grid(
            row=0, column=0, sticky="nsew", padx=(
                0, 10), pady=(
                0, 5))  # Reduced bottom padding from 10 to 5
        ...
        self.toggle_button = ctk.CTkButton(
            ...
            fg_color=self.colors["status_green"],  # Changed to green when off
...
Special thanks to all contributors and users who have helped improve DragonVoice.""")
        ...
        links_frame.pack(fill="x", padx=15, pady=(10, 5))
    ...
    def create_status_indicator(self, parent, label, value, color, index):
        ...
        indicator.grid(row=row, column=col, padx=5, pady=5, sticky="nsew")
        ...
        setattr(self, f"status_indicator_{index}", indicator)
        setattr(self, f"status_value_{index}", value_widget)
    ...
    def create_config_tab(self):
        ...
        # Main container
        config_frame = ctk.CTkFrame(self.tab_view.tab("Configuration"), fg_color=self.colors["bg_dark"])
        ...
        # Title frame
        title_frame = ctk.CTkFrame(config_frame, fg_color=self.colors["bg_dark"], height=50)
        ...
        # Add Whisper configuration frame
        from whisper_config import WhisperConfigFrame
        whisper_config = WhisperConfigFrame(
            settings_container,
            self.colors,
            self.fonts,
            self.config,
        ...
        whisper_config.grid(row=0, column=0, sticky="ew", pady=10)

        # Add separator - use bg_light as fallback if separator key doesn't exist
        separator_color = self.colors.get("separator", self.colors["bg_light"])
        separator = ctk.CTkFrame(settings_container, fg_color=separator_color, height=2)
        separator.grid(row=1, column=0, sticky="ew", pady=20)

        # Placeholder for future configuration sections
        placeholder = ctk.CTkLabel(
            settings_container,
            text="Additional configuration settings will be available in future updates.",
            font=self.fonts["small"],
            text_color=self.colors["text_dim"]
        )
        placeholder.grid(row=2, column=0, pady=20)

        # Configure grid weights for the tab
        self.tab_view.tab("Configuration").grid_columnconfigure(0, weight=1)
        self.tab_view.tab("Configuration").grid_rowconfigure(0, weight=1)
