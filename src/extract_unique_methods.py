#!/usr/bin/env python3
"""
Script to extract unique methods from dragon_gui.py and create a clean version
"""
import re
import os
from datetime import datetime

def extract_unique_methods(content):
    """Extract unique methods from the content"""
    ...
    for match in methods:
        ...
        # Only store the first occurrence of each method
...
def extract_class_structure(content):
    """Extract the class structure without method bodies"""
    ...
    # Replace method bodies with pass statements
    method_pattern = re.compile(r'^(\s+def\s+\w+\s*\([^)]*\):).*?(?=^\s*def|\Z)', re.MULTILINE | re.DOTALL)
    class_structure = method_pattern.sub(r'\1\n        pass\n\n', class_content)
...
def create_clean_file(input_file, output_file):
    """Create a clean version of dragon_gui.py with unique methods"""
    try:
        ...
        class_structure = extract_class_structure(content)
        if not class_structure:
            print("Error: Could not find DragonVoiceGUI class definition.")
            return False
        
        print(f"Extracted class structure: {len(class_structure)} bytes")
        
        # Extract unique methods
        unique_methods = extract_unique_methods(content)
        print(f"Extracted {len(unique_methods)} unique methods")
        ...
        # Create the clean content by replacing method placeholders with actual methods
        clean_content = imports_and_constants + "\n\n"
        
        # Add the class definition line
        class_def_line = class_structure.split('\n')[0]
        clean_content += class_def_line + "\n"
        
        # Add the class docstring if present
        docstring_match = re.search(r'^\s+""".*?"""', class_structure, re.MULTILINE | re.DOTALL)
        if docstring_match:
            clean_content += docstring_match.group(0) + "\n\n"
        
        # Add the __init__ method first
        if "__init__" in unique_methods:
            clean_content += unique_methods["__init__"][1] + "\n\n"
        
        # Add all other methods
        for method_name, (line_number, method_code) in sorted(unique_methods.items(), key=lambda x: x[1][0]):
            if method_name != "__init__":
                clean_content += method_code + "\n\n"
...
if __name__ == "__main__":
    ...
    output_file = f'src/dragon_gui_unique_{timestamp}.py'
