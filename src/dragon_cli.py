...
class DragonVoiceCLI:
    ...
    def print_welcome(self):
        ...
        if has_colorama:
            ...
            print(f"{Fore.CYAN}║{Style.BRIGHT} 🐉 DragonVoice CLI - Voice-to-Text Assistant{Style.RESET_ALL}{Fore.CYAN} ║{Style.RESET_ALL}")
            print(f"{Fore.CYAN}╠{'═' * 48}╣{Style.RESET_ALL}")
            print(f"{Fore.CYAN}║{Fore.GREEN} Hold Enter key{Fore.WHITE} to start recording{Style.RESET_ALL}{' ' * 24}{Fore.CYAN}║{Style.RESET_ALL}")
            print(f"{Fore.CYAN}║{Fore.GREEN} Release Enter{Fore.WHITE} to stop and process recording{Style.RESET_ALL}{' ' * 13}{Fore.CYAN}║{Style.RESET_ALL}")
            print(f"{Fore.CYAN}║{Fore.YELLOW} Type '{Fore.WHITE}help{Fore.YELLOW}' or '{Fore.WHITE}?{Fore.YELLOW}' for command list{Style.RESET_ALL}{' ' * 16}{Fore.CYAN}║{Style.RESET_ALL}")
            print(f"{Fore.CYAN}╚{'═' * 48}╝{Style.RESET_ALL}")
        else:
            print("\n╔" + "═" * 48 + "╗")
            print("║ 🐉 DragonVoice CLI - Voice-to-Text Assistant ║")
            print("╠" + "═" * 48 + "╣")
            print("║ Hold Enter key to start recording             ║")
            print("║ Release Enter to stop and process recording   ║")
...
Recording Controls:
  ...
  device, d       - Select input device
  gain, g         - Adjust input gain
  noise, n        - Configure noise reduction
  continuous, c   - Toggle continuous recording mode
  quick, q        - Quick mode: record, transcribe, and send to all chatbots
  paste, p        - Paste text from clipboard to all chatbots

Transcription:
  transcribe, t   - Transcribe last recording
  realtime, rt    - Toggle real-time transcription
  translate, tr   - Translate transcription
  format, f       - Set output format
  prompt, pr      - Set custom prompt

Chatbot Integration:
  chatbot, cb     - Configure chatbot input settings
  calibrate, cal  - Calibrate chatbot coordinates
  profile, prof   - Load a saved chatbot profile
  retry           - Retry sending to failed chatbots
  add_preset      - Add preset chatbot coordinates

Settings:
  config, cf      - Configure settings
  language, l     - Set recognition language
  quality, q      - Set recording quality
  silence, si     - Configure silence detection
...
"""
    ...
    def process_command(self, command: str) -> bool:
        ...
        if command in ['record', 'r']:
            ...
            self.toggle_pause()
        elif command in ['device', 'd']:
            self.select_device()
        elif command in ['gain', 'g']:
            self.adjust_gain()
        elif command in ['noise', 'n']:
            self.configure_noise_reduction()
        ...
        # Transcription controls
            ...
            self.transcribe_audio()
        elif command in ['realtime', 'rt']:
            self.toggle_realtime_transcription()
        elif command in ['translate', 'tr']:
            self.configure_translation()
        elif command in ['format', 'f']:
            self.set_output_format()
        elif command in ['prompt', 'pr']:
            self.set_custom_prompt()
        elif command in ['clear_coordinates']:
            self.clear_coordinates()
        elif command in ['add_preset']:
            self.add_preset_coordinates()
            
        # Chatbot integration
        elif command in ['chatbot', 'cb']:
            self.configure_chatbot_input()
        ...
        # Settings
            ...
            self.configure()
        elif command in ['language', 'l']:
            self.set_language()
        elif command in ['quality', 'q']:
            self.set_recording_quality()
        elif command in ['silence', 'si']:
            self.configure_silence_detection()
            
        # System commands
        elif command in ['devices']:
            self.list_devices()
            ...
            self.print_colored("Unknown command. Type 'help' or '?' for available commands.", "yellow")
    ...
    def main_loop(self):
        ...
        self.print_colored("\nReady to record. Press and hold Enter to start recording, release to stop.", "cyan", "bright")
        ...
        while True:  # Loop forever until explicitly quit
            try:
                ...
                if self.config.get("continuous_mode", False):
                    ...
                    cont_status = "CONT_OFF"
                chatbot_status = f"CHATBOTS: {len(self.config.get('chatbot_input', {}).get('coordinates', []))}"
                prompt = f"[{status} | {cont_status} | {chatbot_status}] Hold Enter to record (or type command, help=?): "
    ...
    def record_with_enter_key(self):
        ...
        try:
            ...
            # Clear the terminal line to prepare for recording feedback
            sys.stdout.write("\r" + " " * 80 + "\r")
            sys.stdout.flush()
            
            self.print_colored("Waiting for Enter key press...", "cyan", end="\r")
            ...
            self.print_colored("▶ RECORDING... (Release Enter to stop)", "green", "bright", end="\r")
            self.recording = True
            self.is_paused = False
            ...
            # Display live recording indicator while Enter is held
            ...
            sys.stdout.write("\r" + " " * 80 + "\r")  # Clear the line
            self.print_colored("⏹ Recording stopped, processing...", "yellow")
            self.stop_recording()
            
            # Process the recording
            if self.save_audio():
                transcript = self.transcribe_audio()
                if transcript and self.confirm_or_edit_transcription():
                    self.input_to_chatbots()
    ...
    def _show_recording_indicator(self):
        """Display a live recording indicator"""
        indicators = ["▶ RECORDING ", "▶ RECORDING. ", "▶ RECORDING.. ", "▶ RECORDING... "]
        i = 0
        
        while self.recording and not self.is_paused:
            indicator = indicators[i % len(indicators)]
            duration = time.time() - self.recording_start_time
            sys.stdout.write(f"\r{indicator} {duration:.1f}s (Release Enter to stop)" + " " * 10)
            sys.stdout.flush()
            i += 1
            time.sleep(0.3)
    ...
    def quick_mode(self):
        ...
        try:
            ...
            if not self.config["chatbot_input"]["coordinates"]:
                self.print_colored("No chatbot coordinates configured. Please calibrate with command '13'.", "yellow")
                return
                
            # Start recording
            self.print_colored("QUICK MODE: Starting recording. Speak your question clearly.", "cyan")
            self.print_colored("Press Enter when done speaking to automatically transcribe and send to all chatbots.", "yellow")
            
            # Start recording
            if not self.start_recording():
                self.print_colored("Failed to start recording. Please try again.", "red")
                return
            ...
            try:
                ...
                self.print_colored("Recording cancelled by user.", "yellow")
                self.recording = False
                return
            
            # Stop recording and transcribe
            self.print_colored("Processing your recording...", "cyan")
            self.stop_recording()  # This will also trigger transcription
            
            # Skip confirmation and directly send to chatbots
            if self.last_transcription:
                # Copy to clipboard
                pyperclip.copy(self.last_transcription)
                
                # Show what will be sent
                self.print_colored(f"Sending to {len(self.config['chatbot_input']['coordinates'])} chatbots:", "green")
                self.print_colored(self.last_transcription, "cyan")
                
                # Ask if user wants to send to chatbots
                send_to_chatbots = input("\nSend this transcription to chatbots? (y/n): ").strip().lower()
                if send_to_chatbots != 'y':
                    self.print_colored("Sending cancelled.", "yellow")
                    return
                
                # Use fast mode for input to chatbots
                # Temporarily modify the delay between inputs for quick mode
                original_delay = self.config["chatbot_input"]["delay_between_inputs"]
                self.config["chatbot_input"]["delay_between_inputs"] = min(original_delay, 0.2)  # Cap at 0.2 seconds
                
                # Input to chatbots with skip_confirmation=True
                self.input_to_chatbots(skip_confirmation=True)
                
                # Restore original delay
                self.config["chatbot_input"]["delay_between_inputs"] = original_delay
                
                # Ready for next quick recording
                self.print_colored("Ready for next quick recording. Press 'q' to record again.", "cyan")
            else:
                self.print_colored("No transcription available or transcription was cancelled. Try recording again.", "yellow")
                
        except Exception as e:
            logger.error(f"Error in quick mode: {e}")
            self.print_colored(f"Error in quick mode: {e}", "red")
            # Make sure recording is stopped if an error occurs
            if self.recording:
                self.recording = False
                self.print_colored("Recording stopped due to error.", "yellow")
                
            # Restore original delay if it was changed
